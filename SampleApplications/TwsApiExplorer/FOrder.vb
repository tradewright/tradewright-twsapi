...
#Region "Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.CancelItButton.Font = New System.Drawing.Font("Segoe UI", 9.75!, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point)
        Me.CancelItButton.ForeColor = System.Drawing.Color.DimGray
        Me.CancelItButton.Location = New System.Drawing.Point(6, 120)
        ...
        Me.PrimaryExchangeText.Text = "ARCA"
        '
        'RightText
        '
...
#End Region
    ...
    Private ReadOnly mFormTitles As New Dictionary(Of OrderFormMode, String) From {
        ...
        {OrderFormMode.WhatIf, "'What If' Order"},
    ...
    Public ReadOnly Property ExerciseAction() As ExerciseAction
        Get
            Return [Enum].Parse(Of ExerciseAction)(ExerciseActionText.Text, True)
        End Get
    End Property
    ...
    Public ReadOnly Property SnapshotMktData() As Boolean
        Get
            Return SnapshotMktDataCheck.CheckState = CheckState.Checked
        End Get
    End Property
    ...
    Private Sub algoParamsButton_Click(sender As Object, e As System.EventArgs) Handles AlgoParamsButton.Click
        Using f As New FAlgoParams
            ...
            f.Init(OrderInfo.AlgorithmStrategy, OrderInfo.AlgorithmParameters)
            Dim res = f.ShowDialog()
            If res = DialogResult.OK Then
                OrderInfo.AlgorithmStrategy = f.AlgoStrategy
                OrderInfo.AlgorithmParameters = f.AlgoParams
            End If
        End Using
    End Sub
    ...
    Private Sub oKButton_Click(sender As Object, e As EventArgs) Handles OkButton.Click
        ...
        ContractInfo.ContractId = CInt(ConIdText.Text)
        ContractInfo.Symbol = SymbolText.Text.ToUpper

        If Not IBAPI.SecurityTypes.TryParse(SecTypeText.Text, ContractInfo.SecurityType, ignoreCase:=True) Then
        ...
        If Not IBAPI.OptionRights.TryParse(RightText.Text, ContractInfo.OptionRight, ignoreCase:=True) Then
            mErrorsText.DisplayMessage($"Invalid option right - must be one of {String.Join(", ", IBAPI.OptionRights.ExternalNames)}")
            DialogResult = DialogResult.No
        End If
        ContractInfo.Multiplier = If(MultiplierText.Text = "", 1, CDbl(MultiplierText.Text))
        ContractInfo.Exchange = ExchangeText.Text.ToUpper
        ContractInfo.PrimaryExchange = PrimaryExchangeText.Text.ToUpper
        ...
        If Not IBAPI.OrderActions.TryParse(ActionText.Text, OrderInfo.Action, ignoreCase:=True) Then
            ...
            DialogResult = DialogResult.No
        End If
        OrderInfo.TotalQuantity = CDec(QuantityText.Text)
        If Not IBAPI.OrderTypes.TryParse(OrderTypeText.Text, OrderInfo.OrderType, ignoreCase:=True) Then
            ...
            DialogResult = DialogResult.No
        End If
        OrderInfo.LimitPrice = IBAPI.NullableDoubleFromString(LimitPriceText.Text)
        OrderInfo.AuxPrice = IBAPI.NullableDoubleFromString(AuxPriceText.Text)
        OrderInfo.CashQuantity = IBAPI.NullableDoubleFromString(CashQtyText.Text)
        ...
        mMarketDataType = CType(MarketDataTypeCombo.SelectedIndex + 1, MarketDataType)
    ...
    Private Sub initialize()
        ...
        EndDateTimeText.Text = Format(Now().ToUniversalTime, "yyyy-MM-dd HH:mm:ss")
    ...
    Private Sub smartComboRoutingParamsButton_Click(sender As System.Object, e As System.EventArgs) Handles SmartComboRoutingParamsButton.Click
        ...
        f.Init(OrderInfo.SmartComboRoutingParameters, "Smart Combo Routing Params")
        Dim res = f.ShowDialog()
        If res = DialogResult.OK Then
            OrderInfo.SmartComboRoutingParameters = New List(Of TagValue)(f.Options)
        End If
        f.Hide()
    End Sub
